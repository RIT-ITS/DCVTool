...
function init() {
    ...
    function autoCalculate(id) {
        ...
        let group = table.rows(function (idx, data, node) {
            return data.room_id == id ?
                true : false;
        });

        //loop though and add together group data
        for (let j = 0; j < group.data().length; j++) {
            group.data()[j].pr_percent = parseFloat(1 / group.data().length);
    ...
    function loadTable() {
        ...
        $("#airflowTable").DataTable({
            ...
            rowGroup: {
                ...
                endRender: function (rows, group) {
                    let groupXrefs = rows.data().pluck('zone_xrefs')[0];
                    //add a class to the group rows
                    rows.nodes().to$().addClass("length-" + rows[0].length);
                    ...
                    switch (sortMode) {
                        case 0:
                            ...
                            addCell(tr, "Total RaAz: " + Math.round(areaOaSum), 1);

                            if (populationSum != rows.data()[0]['room_population'] + rows.data()[0]['uncert_amt']) {
                                addCell(tr, "Total Population: " + Math.round(populationSum), 1, "makeRed");
                            }
                            else {
                                addCell(tr, "Total Population: " + Math.round(populationSum), 1);
                            }

                            addCell(tr, "Total RpPz Max: " + peopleOaSum.toFixed(1), 6);
                            break;
                        case 1:
                            addCell(tr, group, 1, 'airflow-group-start');
                            ...
                            addCell(tr, "Total Population: " + Math.round(populationSum), 1);
                            addCell(tr, "Total RpPz Max: " + Math.round(peopleOaSum), 6);
                            break;
                    }
            ...
            columnDefs: [
                ...
                {
                    targets: 3,
                    data: { "auto_mode": "auto_mode", "reservable": "reservable", "room_xrefs": "room_xrefs", "occ_sensor": "occ_sensor", "occ_stdby_allowed": "occ_stdby_allowed" },
                    //this gives us a span that we can replace with a on/off dropdown
                    render: function (data) {
                        if (data.auto_mode == 0) {
                            if (data.reservable == 1 && data.occ_sensor == 1 && data.occ_stdby_allowed == 1 && data.room_xrefs.length == 1) {
                                return `<span class="dropdown">On</span>`
                            }
                            else {
                                return `<span class="dropdown">Off</span>`
                            }
                        }
                        else {
                            if (data.auto_mode == 1) {
        ...
        if ((configOptions.response.docs.canEditAll == 'on' || configOptions.response.docs.canEditAirflowZones == 'on') && configOptions.response.docs.current_user.role * 1 > 1) {
            table.button().add(5, {
                extend: 'collection',
                text: 'Edit',
                autoClose: true,
        ...
        table.on('select', function (e, dt, type, indexes) {
            ...
            if (cellSelected == false && editMode == true) {
                ...
                if (document.querySelector(".selected .dropdown")) {
                    if (dat.auto_mode == 1) {
                        document.querySelector(".selected .dropdown").innerHTML = `<select style="border: 1px solid #aaa" class="form-control input" style='width: 100%;' class='form-select' name="row-dropdown">
    ...
    //deletion and auto-calculate are disabled on this table

    //if the user clicks on a trash icon, bring up the dialog box to confirm
    $("#airflowTable tbody").on('click', '.fa-calculator', function () {
        selectedGroup = parseInt(this.parentNode.attributes.room.value);
        openCalcModal();
    })

    //called if the user cancels the deletion
    function cancelDelete() {
        let dialogElement = document.querySelector("#alertAirflowDel");
        selectedRow = null;
        dialogElement.close();
    }
    ...
    function openCalcModal() {
        if (document.querySelector("#building").value > 0) {
            document.querySelector(".alertTextAuto").innerHTML = "<p>Recalculate percentages for Room " + selectedGroup + " ?</p><p>This will overwrite any manual edits to this group.</p>";
            document.querySelector("#alertAirflowAuto").showModal();
        }
    }

    //cancel autocalculation
    function cancelAuto() {
        let dialogElement = document.querySelector("#alertAirflowAuto");
        dialogElement.close();
    }
    ...
    //Users are unable to add new data to this table. However, the code remains should it be required
    ...
    document.querySelector("#cancelAirflowDel",).addEventListener('click', cancelDelete);

    document.querySelector("#confirmAirflowDel",).addEventListener('click', confirmDelete);
    ...
    document.querySelector("#cancelAirflowAuto",).addEventListener('click', cancelAuto);

    document.querySelector("#confirmAirflowAuto",).addEventListener('click', confirmAuto);
